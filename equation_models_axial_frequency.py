...
def convert_to_engineering_units(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    # convert E columns to GPa if they look like Pa
...
class GAMWrapper(BaseEstimator, RegressorMixin):
    ...
    def equation_text(self, feature_cols):
        return (
            "f_axial = beta0 + s1(E_fixed_GPa) + s2(rho_fixed) + s3(nu_fixed) + "
            "s4(E_free_GPa) + s5(rho_free) + s6(nu_free)"
        )
...
def build_models():
    models = {
        ...
        "Polynomial_deg2": Pipeline([
            ("poly", PolynomialFeatures(degree=2, include_bias=False)),
            ("model", Ridge(alpha=1.0, random_state=RANDOM_STATE)),
        ]),
        "Polynomial_deg3": Pipeline([
            ("poly", PolynomialFeatures(degree=3, include_bias=False)),
            ("model", Ridge(alpha=1.0, random_state=RANDOM_STATE)),
        ]),
    }
    if HAS_SPLINE:
        models["Piecewise_ElasticNet"] = Pipeline([
            ("spline", SplineTransformer(degree=1, n_knots=4, include_bias=False)),
            ("model", ElasticNet(alpha=0.0005, l1_ratio=0.3, max_iter=50000, random_state=RANDOM_STATE)),
        ])
    if HAS_PYGAM:
...
def equation_from_linear_model(name, model, feature_cols):
    ...
    pieces = [f"f_axial = {format_number(intercept)}"]
    for c, feat in zip(coef, feature_cols):
        sign = "+" if c >= 0 else "-"
        pieces.append(f" {sign} {format_number(abs(c))}*{feat.replace(' ', '_')}")
    return "".join(pieces)
...
def equation_from_polynomial_pipeline(name, pipe, feature_cols):
    ...
    for c, n in zip(coef, names):
        if abs(c) < 1e-12:
            continue
        n2 = n.replace(" ", "_").replace("^", "**").replace(" ", "")
        if " " in n2:
            n2 = n2.replace(" ", "*")
        sign = "+" if c >= 0 else "-"
        terms.append(f" {sign} {format_number(abs(c))}*{n2}")
    return f"f_axial = {format_number(intercept)}" + "".join(terms)


def equation_from_piecewise_pipeline(pipe, feature_cols):
    spline = pipe.named_steps["spline"]
    model = pipe.named_steps["model"]
    basis_names = spline.get_feature_names_out(feature_cols)
    coef = model.coef_.ravel()
    intercept = float(np.ravel(model.intercept_)[0]) if np.ndim(model.intercept_) else float(model.intercept_)
    eq = [
        "f_axial = " + format_number(intercept),
...
# rename columns for prettier equations
...
for name, model in models.items():
    ...
    fitted_model = clone(model).fit(X_train.values, y_train.values)
...
# refit simple models with named columns where sklearn can preserve names for polynomial features
named_fitted = {}
for name, model in models.items():
...
with open(OUTPUT_DIR / "equations.txt", "w", encoding="utf-8") as f:
    ...
    f.write("Units used in equations: E in GPa, density in original dataset units, nu dimensionless.\n\n")
    for name, eq in equations:
        f.write(f"[{name}]\n{eq}\n\n")

# =========================================================
# PARITY PLOT FOR BEST EQUATION MODEL
# =========================================================
...
plt.xlabel("Actual axial frequency (Hz)")
plt.ylabel("Predicted axial frequency (Hz)")
plt.title(f"Parity plot - best equation model ({best_name})")
plt.legend()
...
if "GAM" in named_fitted:
    ...
    for i, feat in enumerate(feature_cols_eq):
        ...
        axes[i].set_title(f"GAM partial effect: {feat}")
        axes[i].set_xlabel(feat)
        axes[i].set_ylabel("Effect on f_axial (Hz)")
        axes[i].legend()
